...
public class RoomForSEM extends RoomForProtocol<int[]>{
	...
	public void run() {
		...
		agent.sendMessage(player1, parm1[3], C1);
		agent.sendMessage(player2, parm2[3], C2);
		agent.sendMessage(player1, parm1[3], new Object[]{player2,parm2[3]});
		agent.sendMessage(player2, parm2[3], new Object[]{player1,parm1[3]});
		agent.sendMessage(player1, parm1[3], A);
		agent.sendMessage(player2, parm2[3], B);		
	}
